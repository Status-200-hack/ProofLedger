...
export default function NavBar() {
  ...
  return (
    <nav className="sticky top-0 z-30 border-b border-white/10 bg-white/80 px-4 py-3 sm:px-6 sm:py-4 backdrop-blur-md dark:border-white/5 dark:bg-slate-950/80">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <button
            type="button"
            className="inline-flex items-center justify-center rounded-full border border-slate-200 bg-white/80 p-2 text-slate-700 shadow-sm transition hover:border-slate-300 hover:bg-white dark:border-slate-700 dark:bg-slate-900/80 dark:text-slate-100 dark:hover:border-slate-500 md:hidden"
      ...
      {/* Mobile menu */}
      {isMobileOpen && (
        <div className="mx-auto mt-3 max-w-6xl rounded-2xl border border-white/10 bg-white/95 p-4 text-sm shadow-lg backdrop-blur-md dark:border-white/5 dark:bg-slate-950/95 md:hidden">
          ...
          <div className="mt-4 border-t border-slate-200/70 pt-4 dark:border-slate-800">
            {!isMounted ? (
              <div className="flex w-full items-center justify-center rounded-full bg-slate-200 px-4 py-3 text-xs font-semibold text-slate-600 shadow-sm animate-pulse">
                Loading...
              </div>
            ) : !isConnected ? (
              <button
                ...
                disabled={isConnecting || connectors.length === 0}
                className="flex w-full items-center justify-center rounded-full bg-indigo-600 px-4 py-3 text-xs font-semibold text-white shadow-sm transition hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-60"
              >
                {isConnecting ? "Connecting..." : "Connect Wallet"}
              </button>
            ) : (
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="flex w-full items-center justify-center rounded-full bg-red-500 px-4 py-3 text-xs font-semibold text-white shadow-sm transition hover:bg-red-600"
                >
                  <span className="mr-1.5 text-[12px]" aria-hidden="true">
                    ⏏
                  </span>
                ...
                <span className="flex w-full items-center justify-center rounded-full border border-slate-300 bg-white px-4 py-3 text-[11px] font-mono font-semibold text-slate-800 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100">
                  {shortAddress}
                </span>
              </div>
            )}
